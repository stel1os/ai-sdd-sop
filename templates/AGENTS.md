...
It is **committed** — keep machine-specific paths and personal tooling notes out of it.
...
**Repo:** [GitHub URL]
**Spec:** `SPEC.md` (repo root, committed)
**SOP:** https://github.com/stel1os/ai-sdd-sop (v1.4)
...
Tiers are capability levels, not model names: **fast / standard / frontier** (as of 2026-06: fast = Haiku, standard = Sonnet, frontier = Opus/Fable). "Per task" means the plan doc's complexity classification picks the tier — see the SOP's Model tiers section.
...
**Rejection loop:** a ❌ produces a findings document (file:line + criterion violated) — that document, not the reviewer's reasoning, goes back to a fresh Developer (or Test Designer for pre-review) context. The same reviewer role re-reviews. Two failed cycles on one task → stop and escalate to the user.
